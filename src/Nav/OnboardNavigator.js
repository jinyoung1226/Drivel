...
import {regions ,driveStyle, driveView, driveWith} from '../assets/onboardingData/onBoardingData';
...
const OnboardingPage = ({ navigation }) => {
  ...
  const ButtonDisabled = () => {
    ...
    if (step === 2) {
      return (
        selectedDriveWith.length === 0
      );
    }
    if (step === 3) {
      return (
        selectedDriveView.length === 0
      );
    }
    if (step === 4) {
      return (
        selectedDriveStyle.length === 0
      );
    }
  };
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: colors.BG}}>
      ...
      {step === 1 ? (
        ...
        <View style={{padding: 16, flex:1, justifyContent:'center', alignItems:'center'}}>
          <Text style={[textStyles.H1, {color: colors.Gray10, textAlign:'center'}]}>
          이제 본인의 취향에 맞는{'\n'}드라이브 코스를{'\n'}추천 받을 수 있어요!
          </Text>
        </View>
        )
      ...
      <View style={{padding:16}} >
        <CustomButton title={step < 4 ? '다음' : step== 4 ? ('완료하기') : ('시작하기')} onPress={handleNextStep} disabled={ButtonDisabled()}/>
      </View>
    </SafeAreaView>
  );
};
