...
const MeetTab = ({navigation, route}) => {
  useLayoutEffect(() => {
    ...
    if (routeName === "Meet" ||  routeName === undefined) {
      navigation.setOptions({ tabBarStyle : {height: Platform.OS === 'ios' ? 93 : 70, display: 'flex'}});
    } else {
  ...
  return (
    <Stack.Navigator>
      <Stack.Screen name="Meet" component={MeetMain} options={{headerShown: false}}/>
      <Stack.Screen name="MeetCreate" component={MeetCreate} options={{title: "모임 만들기", headerTitleAlign: 'center', headerTitleStyle: [textStyles.H2, {color: colors.Gray10}]}}/>
