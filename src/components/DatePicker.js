...
const MONTH_ITEMS = Array.from({ length: 12 }, (_, i) => `${(i + 1).toString().padStart(2, '0')}월`);
const DAY_ITEMS = Array.from({ length: 31 }, (_, i) => `${(i + 1).toString().padStart(2, '0')}일`);

const DatePicker = ({ value, onChange }) => {
  const yearRef = useRef(null);
  const monthRef = useRef(null);
  const dayRef = useRef(null);
  const [selectedYearIndex, setSelectedYearIndex] = useState(YEAR_ITEMS.indexOf(`${value.getFullYear()}년`));
  const [selectedMonthIndex, setSelectedMonthIndex] = useState(value.getMonth());
  const [selectedDayIndex, setSelectedDayIndex] = useState(value.getDate() - 1);
  const [isScrolling, setIsScrolling] = useState({ year: false, month: false, day: false });
  ...
  const handleScroll = (items, key) => {
    return (event) => {
      const index = Math.round(event.nativeEvent.contentOffset.y / BUTTON_HEIGHT);
        ...
        {fullItems.map((item, index) => {
          ...
          return (
            <View key={index} style={styles.button}>
              <Text style={[styles.buttonLabel, isSelected && styles.selectedButtonLabel]}>{item}</Text>
            </View>
          );
        })}
      </ScrollView>
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#ffffff','rgba(255, 255, 255, 0)','rgba(255, 255, 255, 0)','rgba(255, 255, 255, 0)', '#ffffff']} style={styles.overlay} pointerEvents="none">
...
const styles = StyleSheet.create({
  container: { flexDirection: 'row', height: BUTTON_HEIGHT * VISIBLE_ITEMS_COUNT, width: '90%', alignSelf: 'center'},
  scrollView: { flex: 1 },
  button: { height: BUTTON_HEIGHT, justifyContent: 'center', alignItems: 'center' },
