        ...
        <TouchableOpacity style={{flex:1}} onPress={()=>{setModalVisible(!modalVisible)}} />
          <View style={{flexDirection:'row'}} >
            ...
            <View style={{flex:1}}>
              ...
              <Pressable
                style={({pressed}) => [{backgroundColor: pressed ? colors.Gray02 : colors.white,  borderRadius:10, padding:16}]}
                onPress={()=> {setModalVisible(!modalVisible);}}  
              >
                <Text style={[textStyles.H4, {color:colors.Gray10, alignSelf:'center'}]}>취소</Text>
              </Pressable>
            </View>
            <TouchableOpacity style={{width:32}} onPress={()=>{setModalVisible(!modalVisible)}} />
          </View>
        <TouchableOpacity style={{height:32}} onPress={()=>{setModalVisible(!modalVisible)}} />
      </View>
    </Modal>
  );
};
