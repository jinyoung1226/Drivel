...
const ReportPage = ({navigation, route}) => {
  ...
  useEffect(() => {
    if (isReportTextActive) {
      const customIndex = selectedReportReason.findIndex(reason => typeof reason === 'string' && reason.startsWith('기타:'));
      if (customIndex !== -1) {
        const updatedReasons = [...selectedReportReason];
        updatedReasons[customIndex] = `기타: ${reportText}`;
        setSelectedReportReason(updatedReasons);
      } else if (reportText) {
        setSelectedReportReason(prevReasons => [...prevReasons, `기타: ${reportText}`]);
      }
    }
  }, [reportText]);
  ...
  const handleCustomChipPress = () => {
    ...
    if (isReportTextActive) {
      setSelectedReportReason(prevReasons => prevReasons.filter(reason => !reason.startsWith('기타:')));
  ...
  return (
    <View style={{flex: 1, backgroundColor: colors.BG}}>
      ...
      <Text style={[textStyles.H3, {color: colors.Gray10, alignSelf: 'center'}]}>
        {/* {targetId} */}
        신고 사유를 선택해 주세요
      </Text>
      <View style={{height: 8}} />
      ...
      >
        <View style={{padding:16}}>
          ...
          {isReportTextActive &&
            <TextInput
              style={[textStyles.C4, {color: colors.Gray10, borderRadius:10, borderWidth:1, borderColor: colors.Gray03, paddingHorizontal: 16, height:190, textAlignVertical: 'top'}]}
              placeholder={'다른 불쾌한 일을 겪으셨나요?\n신고 내용을 입력해주세요 (최대 200자)'}
