    ...
    <View key={index} style={{flexDirection: 'row'}}>
      ...
      <View>
        ...
        <Text
          style={[
            textStyles.B4,
            {color: colors.Gray06, width: width - 122},
          ]}
          numberOfLines={2}>
          {description}
        </Text>
  ...
  return (
    <View style={{width: width}}>
      <View style={{padding: 16}}>
        ...
        <View style={{height: 16}} />
        {renderMemberInfo(item.meetingInfo.masterInfo.imagePath, item.meetingInfo.masterInfo.nickname, item.meetingInfo.masterInfo.description)}
      </View>
      <GrayLine />
      <View style={{padding:16}}>
        <View style={{height: 16}} />
        <Text style={[textStyles.H4, {color: colors.Gray10}]}>참여자 ({item.meetingInfo.participantsInfo.membersInfo.length}/{item.meetingInfo.condition.capacity})</Text>
        <View style={{height: 16}} />
        {item.meetingInfo.participantsInfo.membersInfo.map((member, index) => (renderMemberInfo(member.imagePath, member.nickname, member.description, index)))}
      </View>
    </View>
  );
};
