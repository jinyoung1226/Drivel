...
import { View, Text, Button, StyleSheet, TouchableOpacity, Alert, TextInputComponent, ScrollView } from 'react-native';
...
const Register = ({ navigation }) => {
  ...
  const signUp = async () => {
    try {
      const response = await api.post('/auth/signUp', { email: email, password: password });
  ...
  const verifyAuthCode = async () => {
    ...
    try {
      const response = await api.post('/mail/check', { email: email, code: authCode });
      if (response.status === 200) {
        Alert.alert(response.data.message);
    ...
    <View style={{flex: 1, backgroundColor: colors.BG}}>
      ...
      <KeyboardAwareScrollView>
        <View style={{padding:16}}>
          <Text style={[textStyles.H4, {color: colors.Gray10}]}>이메일 주소</Text>
          ...
          <CustomInput
            ...
            showButton={true}
            buttonIcon={({buttonDisabled}) => (<Text style={[textStyles.H5, {color: buttonDisabled ? '#C5C5C6': '#5168F6'}]}>{authRequestText}</Text>)}
          ...
          {isSended && !isAuthorized &&
          <View>
            <Text style={[textStyles.H4, {color: colors.Gray10}]}>인증번호</Text>
            <View style={{height:16}}/>
            <CustomInput
              ...
              showButton={true}
              onButtonPress={()=>{verifyAuthCode()}}
              buttonIcon={({buttonDisabled}) => (<Text style={[textStyles.H5, {color: buttonDisabled ? '#C5C5C6': '#5168F6'}]}>확인</Text>)}
              editable={isSended && !isAuthorizing && !isAuthorized}
              buttonDisabled={isAuthorizing || isAuthorized || authCode.length == 0}
              showTimer={true}
          ...
          <CustomInput
            ...
            onButtonPress={()=>{setIsSecurePassword(!isSecurePassword)}}
          ...
          />
          <Text style={[textStyles.H4, {color: colors.Gray10}]}>비밀번호 확인</Text>
          <View style={{height:16}}/>
          <CustomInput
            ...
            showButton={true}
            isButtonText={false}
            buttonIcon={<Eye color={isSecurePasswordCheck? "#C5C5C6" : "#4A4A4A"}/>}
            onButtonPress={()=>{setIsSecurePasswordCheck(!isSecurePasswordCheck)}}
      ...
      <CustomButton
        disabled={password==0 || passwordCheck == 0 || !isValidPassword || !isPasswordSame || !isAuthorized}
        onPress={signUp}
        title={'회원가입'}
      />
      </View>
